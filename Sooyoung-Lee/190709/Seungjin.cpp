...
public:
	...
	void calculate(double _real, double _imag, char oper) {
		...
		case '*':
			...
			imag = tmp_real * _imag + tmp_imag * _real;
			break;
		case '/':
			real = (tmp_real*_real + tmp_imag * _imag) / (_real*_real + _imag * _imag);
			imag = (-tmp_real * _imag + tmp_imag * _real) / (_real*_real + _imag * _imag);
			break;
		}
	}
